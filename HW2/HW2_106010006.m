...
t = [0:0.1:10];
syms s
F = (s+2*zeta*wn)*y0/(s^2+2*zeta*wn*s+wn^2);
y1 = ilaplace(F)
%上面算出的y1把*改成.*後,assign成下面的y
y = (3*exp(-2^(1/2)*t).*(2^(1/2)*t + 1))/20;
